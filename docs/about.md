...
请检查是否设置了 `rowHeight` 属性，如果设置了，请确保 `rowHeight` 的值是正确的，否则可能会出现滚动到某一行位置不精确的情况。<span style="color:#ff4d4f;">重要：如果你的行高不固定（例如，文字可以换行之类的），则不要设置行高</span>
...
你可以通过设置 `maxTableWidth` 属性为 `max-content` ，此时每一列的内容会自适应自己的列宽，不会换行。（其余情况下，表格会对每一列进行换行显示）。
...
**8. 如何自定义 `checkbox` `pagination` 等内置组件做二次封装？**
